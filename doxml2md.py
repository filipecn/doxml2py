...
def getInstanceList(className, node, child):
    class_ = getattr(importlib.import_module("doxml2md"), className)
    instList = []
    if node is not None:
...
class Compound:
    def __init__(self, c):
        self.object = c
        try:
            self.name = c.find('compoundname').text
        except:
            self.name = ""
...
class Param:
    def __init__(self, m, p):
        self.type = p.find('type').text
        try:
            self.name = p.find('declname').text
        except:
            self.name = ""
        self.detaileddescription = ""
        try:
...
class Method(Compound):
    def __init__(self, m):
        ...
        self.name = m.find('name').text
        self.object = m
        self.definition = m.find('definition').text
        self.args = m.find('argsstring').text
...
class Class(Compound):
    def __init__(self, cl):
        ...
        print self.name
        try:
            self.includes = cl.find('includes').text
        except:
            self.includes = ""
